...
scripts/1N/compile_and_link.sh
retcode=$?
if [[ $retcode != 0 ]]; then
	exit $failure
fi
...
for input in enroll search
do
	numInputLines=$(cat input/$input.txt | wc -l)
	numLogLines=$(sed '1d' $outputDir/$stem.$input | wc -l)
	if [ "$input" == "search" ] && [ "$numLogLines" -ne 13080 ]; then
		echo "[ERROR] The $outputDir/$stem.$input file has the wrong number of lines.  It should contain 13080 but contains $numLogLines.  Please re-run the validation test."
		exit $failure
	elif [ "$input" == "enroll" ] && [ "$numInputLines" != "$numLogLines" ]; then
		echo "[ERROR] The $outputDir/$stem.$input file has the wrong number of lines.  It should contain $numInputLines but contains $numLogLines.  Please re-run the validation test."
		exit $failure
	fi
	...
	if [ "$numFail" -ne "0" ]; then
		echo -e "\n[WARNING] The following entries in $stem.$input generated non-successful return codes:"
...
libstring=$(basename `ls ./lib/libfrpc_1N_*_?_[cg]pu.so`)
...
using the FRPC public key linked from -
